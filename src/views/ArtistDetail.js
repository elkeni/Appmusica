...
export default function ArtistDetail({ favorites, toggleFavorite, onAddPlaylist }) {
    ...
    useEffect(() => {
        const loadData = async () => {
            setLoading(true);
            try {
                // Obtener datos del artista de la nueva API unificada
                const artistData = await getArtist(id, 'deezer');
                const tracksData = await getArtistTopTracks(id, 'deezer', 20);
                const albumsData = []; // Albums requieren implementación adicional

...
export default function HomeView({ onToggleFavorite, favorites, onAddPlaylist }) {
    ...
    useEffect(() => {
        const loadRecommendations = async () => {
            ...
            setError(null);
            try {
                console.log('🏠 Loading trending tracks for home view...');
                
                // Obtener trending de la nueva API unificada
                const trendingData = await getTrending(50);
                
                if (!trendingData || trendingData.length === 0) {
                    throw new Error('No recommendations available');
                }

                console.log(`✅ Loaded ${trendingData.length} trending tracks`);

                setRecommendations({
                    highlighted: trendingData.slice(0, 15),   // Top trending
                    new: trendingData.slice(15, 30),          // New releases
                    recent: trendingData.slice(30, 45)        // Recent popular
                });
            } catch (err) {

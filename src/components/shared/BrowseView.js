...
export default function BrowseView({ onToggleFavorite, favorites, onAddPlaylist }) {
    ...
    const { getTrending, search } = useMusic();
    ...
    useEffect(() => {
        ...
        const loadData = async () => {
            setLoading(true);
            try {
                console.log('🎵 [BrowseView] Loading trending music...');
                
                // Obtener trending de la nueva API unificada
                const trendingData = await getTrending(40);
                ...
                console.log(`✅ [BrowseView] Loaded ${trendingData.length} trending tracks`);

                // 1. Pick Hero (Random from top 10)
                let hero = null;
                if (trendingData.length > 0) {
                    hero = trendingData[Math.floor(Math.random() * Math.min(10, trendingData.length))];
                    setHeroData(hero);
                }

                // 2. Deduplicate: Filter out Hero from Charts
                const filteredCharts = hero
                    ? trendingData.filter(track => track.id !== hero.id)
                    : trendingData;

                setCharts(filteredCharts);
                
                // 3. Generate fake genres for now (can be enhanced later)
                const fakeGenres = [
                    { id: 1, name: 'Pop', picture_medium: '' },
                    { id: 2, name: 'Rock', picture_medium: '' },
                    { id: 3, name: 'Hip Hop', picture_medium: '' },
                    { id: 4, name: 'Electronic', picture_medium: '' },
                    { id: 5, name: 'Latin', picture_medium: '' },
                    { id: 6, name: 'Jazz', picture_medium: '' },
                ];
                setGenres(fakeGenres);
                
                // 4. Get popular artists by searching
                try {
                    // Buscar música popular para extraer artistas
                    const artistResults = await search('popular music', 15);
                    // Extract unique artists from results
                    const uniqueArtists = [];
                    const seenArtists = new Set();
                    
                    for (const track of artistResults) {
                        if (!seenArtists.has(track.artist) && uniqueArtists.length < 10) {
                            seenArtists.add(track.artist);
                            uniqueArtists.push({
                                id: track.id,
                                name: track.artist,
                                picture_medium: track.image || track.thumbnail,
                                image: track.image || track.thumbnail,
                            });
                        }
                    }
                    setChannels(uniqueArtists);
                } catch (err) {
                    console.warn('⚠️ Could not load artists:', err);
                    setChannels([]);
                }

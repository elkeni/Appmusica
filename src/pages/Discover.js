...
export default function Discover({ items = [], onPlay = () => {}, onOpenPlaylist = () => {} }) {
  return (
    <div className="p-4 mobile-card">
      <h2 className="text-lg font-bold mb-3">Discover</h2>
      {items.length === 0 ? (
        <p className="text-sm text-gray-300">No hay items para mostrar.</p>
      ) : (
        <ul className="space-y-3">
          {items.map((it, idx) => (
            <li key={it.id?.videoId || idx} className="flex items-center justify-between">
              <div>
                <div className="font-semibold">{it.snippet?.title || 'Título'}</div>
              ...
              <div className="flex items-center gap-2">
                ...
                <button className="px-3 py-1 rounded-md glass-fluid-subtle" onClick={() => onOpenPlaylist(it)}>Open</button>
              </div>
            </li>
          ))}
        </ul>
